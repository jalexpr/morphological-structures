...
public final class MorfologyParametersHelper {
    ...
    private static <T extends Number> Map<String, T> createStringParameters(Map<T, String> mapSourcesParameters) {
        Map<String, T> mapParameters = new HashMap<>();
        for (T key : mapSourcesParameters.keySet()) {
            String nameParameterInFile = mapSourcesParameters.get(key);
            String nameParameter = getStringStringMap().get(nameParameterInFile);
            if (nameParameter == null) {
                throw new RuntimeException(nameParameterInFile);
            }
            mapParameters.put(nameParameter, key);
        }
    ...
    private static <T extends Number> void initIdentifiearByParameterClass(
        ...
        try {
            for (Field characteristic : parameterClass.getFields()) {
                if (!"IDENTIFIER".equals(characteristic.getName()) && !"SHIFTBIT".equals(characteristic.getName()) && !"UNFAMILIAR".equals(characteristic.getName())) {
	...
	private static Map<String, String> createMap() {
        ...
        String[] arrProg = {"NOUN", "ADJECTIVE_FULL", "ADJECTIVE_SHORT", "COMPARATIVE", "VERB", "INFINITIVE", "PARTICIPLE_FULL", "PARTICIPLE_SHORT", "GERUND", "NUMERAL", "ADVERB", "NOUN_PRONOUN", "PREDICATE", "PRETEXT", "UNION", "PARTICLE", "INTERJECTION", "ANIMATE", "INANIMATE", "MANS", "FEMININ", "NEUTER", "COMMON", "SINGULAR", "PLURAL", "SGTM", "PLTM", "FIXD", "NOMINATIVE", "GENITIVE", "DATIVE", "ACCUSATIVE", "ABLTIVE", "PREPOSITIONA", "VOATIVE", "GENITIVE1", "GENITIVE2", "ACCUSATIVE2", "PREPOSITIONA1", "PREPOSITIONA2", "ABBREVIATION", "NAME", "SURN", "PART", "GEOX", "ORGN", "TRAD", "SUBX", "SUPR", "QUAL", "APRO", "ANUM", "POSS", "V_EY", "V_OY", "CMP2", "V_EJ", "PERFECT", "IMPERFECT", "TRAN", "INTR", "IMPE", "IMPX", "MULT", "REFL", "PER1", "PER2", "PER3", "PRESENT", "PAST", "FUTURE", "INDICATIVE", "IMPERATIVE", "INCLUSIVE", "EXCLUSIVE", "ACTIVE", "PASSIVE", "INFR", "SLNG", "ARCH", "LITR", "ERRO", "DIST", "QUES", "DMNS", "PRNT", "V_BE", "V_EN", "V_IE", "V_BI", "GERUND_IMPERFECT", "PRDX", "COUN", "COLLECTIVE_NUMERAL", "GERUND_SHI", "AF_P", "INMX", "VPRE", "ANAPHORICPRONOUN", "INIT", "ADJX", "UNCLEARGENDER"};
        //noinspection ConstantConditions
        if (arrFile.length != arrProg.length) { //todo move to test
            throw new RuntimeException("Количество ключей и значений не совпадают");
        }
